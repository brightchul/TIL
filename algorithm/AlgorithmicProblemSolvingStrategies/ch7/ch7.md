...
#### 문제 내용 : [링크](https://algospot.com/judge/problem/read/QUADTREE)



## 7.3 풀이 : 쿼드 트리 뒤집기

가장 무식한 풀이 방법은 주어진 쿼드 트리 압축을 풀어서 실제 이미지를 얻고 상하 반전한 뒤 다시 쿼드 트리 압축하는 것이다. 하지만 크기 제한으로 곧이 곧대로 구현할수는 없고 2가지로 접근 가능하다.

- 큰 입력에 대해서도 동작하는 효율적인 알고리즘을 처음부터 새로 만들기
- 작은 입력에 대해서만 동작하는 단순한 알고리즘으로부터 시작해서 최적화 해 나가기 (이게 더 쉬움)



### 쿼드 트리 압축 풀기

쿼드 트리가 재귀적으로 정의되어 있어 쿼드 트리 압축, 압축 해제는 재귀 호출로 구현하는게 자연스럽다. 문자열 s를 압축 해제해서 NxN 크기의 배열에 저장하는 함수 `decompress()` 를 구현한다면, 

- 기저 사례는 s의 첫 글자가 w나 b인 경우이다. 이때는 배열 전체에 해당 색을 칠하고 종료.
- x라면 나머지 부분을 넷으로 쪼개 재귀 호출한다. 이 때 각 부분의 위치를 알려주는 오프셋도 전달 해야한다.
...
#### 문제 내용 : [링크](https://algospot.com/judge/problem/read/FENCE)
...
#### 문제 내용 : [링크](https://algospot.com/judge/problem/read/FANMEETING)
